-- 
-- packages/contacts/sql/postgresql/upgrade/upgrade-1.2d15-1.2d16.sql
-- 
...
-- we are now using the title column from the acs_objects table and not the title
-- column from the contact_searches table so we remove that column here to not 
-- confuse future programmers

alter table contact_searches drop column title;

-- contacts lists are a simple and super light
-- weight list building mechanism which allows users
-- to share their lists with one another. we do not use
-- groups for this because on sites with 15000+ contacts/users
-- performance is too slow, in part because of the approval
-- process
