ad_page_contract {
	...
	db_0or1row condition_exists_p {
        ...
        if { ![exists_and_not_null owner_id] } {
            ad_complain "[_ contacts.You_have_specified_an_invalid_search_condition]"
        } else { 
		...
		if { [contact::exists_p -party_id $owner_id] } {
		    ad_complain "[_ contacts.You_do_not_have_permission_to_delete_other_peoples_search_conditions]"
		} else {
		    ad_complain "[_ contacts.You_do_not_have_permission_to_delete_this_condition]"
		}
	    }
	}
    }
...
util_user_message -html -message [_ contacts.The_condition_-condition-_was_deleted]
