ad_page_contract {
	...
	if { [db_string get_creator { select creation_user from acs_objects where object_id = :delete_list_id }] ne [ad_conn user_id] || ![contact::owner_p -object_id $delete_list_id -owner_id [ad_conn user_id]] } {
		...
		ad_complain "[_ contacts.You_can_only_delete_list_you_created_and_own]"
	    }
	}
    }
}
...
ad_form \
	...
	{add:text(submit) {label "[_ contacts.Add_List]"}}
    } -validate {
	{title
	...
	{title
	    ...
	    {[_ contacts.List_already_exists_with_this_name]}
	}
    } -on_submit {
...
template::list::create \
    -name "lists" \
    -row_pretty_plural "[_ contacts.lists]" \
    -elements {
        title {
	    label {[_ contacts.List]}
	    link_url_eval $list_url
	}
        sharing {
	    label {[_ contacts.Sharing]}
	    link_url_eval $sharing_url
        }
        members {
	    label {[_ contacts.Members]}
        }
        action {
            label ""
            display_template {
...
db_multirow -extend {list_url delete_url sharing sharing_url} -unclobber lists select_lists {} {
	...
	set sharing "[_ contacts.Public]"
    } else {
	set count [contact::owner_count -object_id $list_id]
	if { $count > 1 } {
	    set sharing "[_ contacts.Shared]"
	} else {
	    set sharing "[_ contacts.Private]"
	}
    }
}
