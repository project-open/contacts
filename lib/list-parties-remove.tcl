ad_page_contract {
	...
	if { ![contact::owner_p -object_id $list_id -owner_id [ad_conn user_id]] } {
	    ad_complain "[_ contacts.You_do_not_own_this_list]"
	}
    }
}
...
if { [llength $party_ids] eq 1 && $list_id ne "" } {
    ...
    # list
    contact::list::member_delete -list_id $list_id -party_id [lindex $party_ids 0]
...
set title "[_ contacts.Remove_from_List]"
...
append form_elements {
    {list_ids:text(checkbox),multiple {label "[_ contacts.Remove_from_Lists]"} {options $list_options}}
}
set edit_buttons [list [list "[_ contacts.lt_Remove_from_selected_Lists]" create]]




ad_form -name remove_from_list \
	...
	db_transaction {
            foreach list_id $list_ids {
                foreach party_id $party_ids {
		    contact::list::member_delete -list_id $list_id -party_id $party_id
                }
            }
	}
    } -after_submit {
